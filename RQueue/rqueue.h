...
private:
     ...
     ******************************************/
    void traverseTreeDelete(Node* node);
    Node* traverseInOrderAssigmentOperator(Node* rhsNode);
    Node* traverseInOrderCopyConstructor(Node* rhsNode);
    void traverseInOrderPrint(Node* node) const;
    void mergeHelper(Node*& node, Node* rhsNode);
    int updateSize(Node* node);
    void arrayTreeConvert(Node* node, Student** nodeArray, int &index);
    void sortArray(Student**& nodeArray, int size);
};
#endif

#ifndef INC_341PROJECT3_RQUEUE_H
#define INC_341PROJECT3_RQUEUE_H

#endif //INC_341PROJECT3_RQUEUE_H
